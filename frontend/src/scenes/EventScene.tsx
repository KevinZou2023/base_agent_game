import { useState } from 'react'
import { SceneRoot } from '../components/SceneRoot'
import { Abs } from '../components/Abs'
import { SceneChrome } from '../components/SceneChrome'
import { useNav } from '../app/nav'
import './EventScene.css'

// 村庄突发 — Figma node 28:1033（询问）→ 100:7668（同意·已接取）.
// 同意/拒绝分支：同意→确认已接取→去采薯莨；拒绝→回村庄。
export function EventScene() {
  const { go } = useNav()
  const [accepted, setAccepted] = useState(false)

  return (
    <SceneRoot bg="/art/master-bg.png">
      <Abs x={670} y={392} w={906} h={1200}>
        <img className="ev-portrait" src="/art/villager-ahua.png" alt="阿花" />
      </Abs>

      <Abs x={1333} y={432} w={1671} h={869} className="ev-panel" />

      {!accepted ? (
        <>
          <Abs x={1780} y={600} w={1093} h={200} className="ev-text">
            今天天气真不错啊，你可以帮我去采一些薯莨吗？我可以给你一些我打的穗子作为报酬。
          </Abs>
          <Abs x={1900} y={850} w={820} h={150} className="ev-quest">
            任务：采集 5 个薯莨　奖励：阿花的穗子 ×3
          </Abs>

          <Abs x={2346} y={1170} w={293} h={116}>
            <button className="ev-btn" onClick={() => go('village')}>
              拒绝
            </button>
          </Abs>
          <Abs x={2684} y={1170} w={293} h={116}>
            <button className="ev-btn ev-accept" onClick={() => setAccepted(true)}>
              同意
            </button>
          </Abs>
        </>
      ) : (
        <>
          <Abs x={1780} y={600} w={1093} h={300} className="ev-text">
            你已接取任务，限时两天，可到左侧任务栏查看。感谢你！期待你这两天的好消息。
          </Abs>
          <Abs x={2346} y={1170} w={631} h={116}>
            <button className="ev-btn ev-accept" onClick={() => go('mountain')}>
              去青翠山采薯莨 ›
            </button>
          </Abs>
        </>
      )}

      <SceneChrome back="village" status={false} />
    </SceneRoot>
  )
}
